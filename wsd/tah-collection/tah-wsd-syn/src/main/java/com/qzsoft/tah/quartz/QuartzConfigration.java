...
import com.qzsoft.tah.service.impl.BjTaskServiceImpl;
...
public class QuartzConfigration {
    ...
    public MethodInvokingJobDetailFactoryBean detailFactoryBean(ScheduledJob task) {// ScheduleTask为需要执行的任务
        ...
        jobDetail.setName("PL-CJ2");// 设置任务的名字
    ...
    public CronTriggerFactoryBean cronJobTrigger(@Qualifier(value = "jobDetail") MethodInvokingJobDetailFactoryBean jobDetail) {
        CronTriggerFactoryBean tigger = new CronTriggerFactoryBean();
        tigger.setJobDetail(jobDetail.getObject());
        tigger.setCronExpression("0 0/1 * * * ? *");// 初始时的cron表达式 默认5分钟
        tigger.setName("PL-CJ2");// trigger的name
        return tigger;
    }
